% trainMLPBP_MI_P2_RNGSeed.m - Function to train MLP classifier
...
% 
% SYNTAX: [W1,W2,b1,b2] = trainMLPBP_MI_P2_RNGSeed(alpha, gamma, epochs);
%
function [W1,W2,b1,b2] = trainMLPBP_MI_P2_RNGSeed(alpha, gamma, epochs)
    ...
    % SET SEED FOR RANDOM NUMBER GENERATION (TESTING PURPOSES ONLY)
    rng(0);
    ...
    for epoch = 1:epochs % iterates for as many epoch in epochs (we pass epochs as a parameter when calling the function)
        ...
        for i = 1:size(PP_TRN, 2) % will run for each column in PP_TRN (PP_TRN, 2); the number 2 represents the 2nd dimension (i.e., columns)
            ...
            target = TT_TRN(:,i); % extract all the rows from the ith column as the target at iteration k
                ...
                delta_W2 = (gamma)*(delta_W2_prev) + (1-gamma)*(-alpha*s2*a1');
                W2 = W2 + delta_W2;

                delta_W1 = (gamma)*(delta_W1_prev) + (1-gamma)*(-alpha*s1*input');
                W1 = W1 + delta_W1;

                delta_b2 = (gamma)*(delta_b2_prev) + (1-gamma)*(-alpha*s2);
                b2 = b2 + delta_b2;

                delta_b1 = (gamma)*(delta_b1_prev) + (1-gamma)*(-alpha*s1);
                b1 = b1 + delta_b1;
                
                % STORE THE CURRENT UPDATES FOR THE NEXT ITERATION
